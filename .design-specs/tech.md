...
- **Language-specific tools**: Kotlin Symbol Processing (KSP) 2.3.2, Kotlin Coroutines (1.10.1) for asynchronous programming.
...
- **Room Persistence Library (2.7.0-alpha11)**: SQLite object mapping for local data storage.
- **MPAndroidChart (v3.1.0)**: Library for displaying data charts.
...
- **Structure**:
    ...
    - **Data Layer**: Repositories for data orchestration, Room database management, and export managers.

### Data Storage
- **Primary storage**: Room Persistence Library (SQLite).
- **Caching**: In-memory caching within Repositories.
- **Data formats**: SQLite (via Room), Kotlin objects, CSV/PNG exports.
...
- **Exporting**: File sharing for measurement data (CSV) and charts (PNG).
